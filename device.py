...
class QDevice:
    ...
    def _run_stored_circ(self):
        ...
        job = simulator.run(self._stored_circs, shots=self.sample_num)
...
def run_cirq_circuit_on_qiskit(circuit, qubits, backend, sample_num, observed_q_label):
    ...
    job = simulator.run(qasm_circuit, shots=sample_num)
    return job.result()
...
def load_ibm_cred():
    ...
    provider = service
    private_provider = service.get_backend("ibmq_qasm_simulator", instance="ibm-q-melbourne/reservations")
    private_provider_default = service.get_backend("ibmq_qasm_simulator", instance="ibm-q-melbourne/internal")
